...
class QueryResponse {
  ...
  QueryResponse(
      this._es, this.index, this.query, Map response, this.skip, this.limit)
      : took = response['took'],
  ...
  static Future<QueryResponse> search(
      Elasticsearch es, String index, Map query, int skip, int limit) async {
    var response = await es
        .index(index)
        .search(query: query..addAll({'from': skip, 'size': limit,}));
